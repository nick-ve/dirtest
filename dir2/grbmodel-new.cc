...
//
// Author   : NvE 17-feb-2006 Utrecht University
...
//            via the new AliMath::PsiExtreme facility.
...
//////////////////////////////////////////////////////////
{
 ...
 Float_t grbnu=-0.03;      // Number of detectable neutrinos per GRB in full IceCube (<0 : no stat. fluct.)
 ...
 Float_t angres=1;        // Detector angular resolution (degrees)
 ...
 Int_t grbpos=1;          // Use the SWIFT grb locations (1) or random ones (0) for bkg studies
 Double_t nrandom=1e4;    // Number of randomised configurations for direct psi P-value determination
 ...
 cout << " Angular resolution (degrees) of the detector : " << angres << endl;
 ...
 Float_t dangup=180;  // Upper value (in degrees) of angular difference histo

 Float_t mup=nmupday*dtwin/(3600.*24.); // Mean no. of upgoing bkg muons in search window
 ...
 if (abin<0) nabins=int(mup*float(ngrbs)/nbkg);
 ...
 for (Int_t igrb=0; igrb<ngrbs; igrb++)
 {
  ...
  for (Int_t imup=0; imup<nmup; imup++)
  {
   ...
   // Smear the angular difference with the Gaussian angular resolution
   if (angres>0) dang=rndm.Gauss(dang,angres);
 ...
 // Compensate statistical fluctuation in number of GRB muons if requested by grbnu<0
 if (grbnu<0)
 {
 ...
 NcMath math;
 // Determine the Bayesian psi value for the time and angular histos
 ...
 Int_t nb=tott->GetNbinsX();
 Float_t nentot=tott->GetEntries();
 Float_t nenbkg=bkgt->GetEntries();

 // Background psi expectation for the recorded time entries
 cout << " *** Expected ideal uniform tott psi : " << math.PsiExtreme(tott) << endl;
 cout << " --- (Unknown) Expected ideal uniform bkgt psi : " << math.PsiExtreme(bkgt) << endl;
 cout << endl;

 Float_t psitott=math.PsiValue(tott,0,0,freq);
 Float_t psibkgt=math.PsiValue(bkgt,0,0,freq);
 ...
 Float_t psitota=math.PsiValue(tota,0,&pdfa,freq);
 Float_t psibkga=math.PsiValue(bkga,0,&pdfa,freq);
 ...
 // Direct determination of the P-value of the observed psi
 Double_t pvalue=math.PsiPvalue(psitott,nrandom,nentot,nb);
 cout << " *** P-value of the observed tott psi : " << pvalue << " Used # of randomisations : " << nrandom << endl;
 cout << endl;
 ...
 cout << " ===> P(bkg|DI)/P(non-bkg|DI) for Time : " << pow(10,-psitdif/10.)
      << " Ang. : " << pow(10,-psiadif/10.) << endl;
 ...
 if (npsi)
 {
  ...
  cout << " ===> P(bkg|DI)/P(non-bkg|DI) for Mean : " << pow(10,-psitdiff/10.)
       << " Median : " << pow(10,-psitdiff2/10.) << endl;
  if (npsi>0)
  {
   ...
   cout << " ===> P(bkg|DI)/P(non-bkg|DI) for Mean : " << pow(10,-psiadiff/10.)
        << " Median : " << pow(10,-psiadiff2/10.) << endl;
  }
  ...
  if (psibkgtdiff2>0 && psitspread>0) cout << " (" << fabs(psibkgtdiff2/psitspread) << " * spread)";
  cout << endl;
  cout << " ===> P(bkg|DI)/P(non-bkg|DI) for Mean : " << pow(10,-psibkgtdiff/10.)
       << " Median : " << pow(10,-psibkgtdiff2/10.) << endl;
  if (npsi>0)
  {
   ...
   if (psibkgadiff2>0 && psiaspread>0) cout << " (" << fabs(psibkgadiff2/psiaspread) << " * spread)";
   cout << endl;
   cout << " ===> P(bkg|DI)/P(non-bkg|DI) for Mean : " << pow(10,-psibkgadiff/10.)
        << " Median : " << pow(10,-psibkgadiff2/10.) << endl;
  }
